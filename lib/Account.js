...
Account.prototype.authorize = function (url, callback) {
    var retval = {
        success: false,
        data: {}
    };
        ...
        if (self.token.access_token === undefined || !self.token.access_token) {
            if (url === null) {
                retval.data = {
                    ...
                    auth_url: "https://www.amazon.com/ap/oa?client_id=" + self.clientId + "&scope=" + scope + "&response_type=code&redirect_uri=http://localhost"
                };

                return callback(null, retval);
            }

            return self.requestAuthorization(url, function (err, data) {
                if (err) {
                    return callback(err);
                }
                ...
                for (var key in data.data) {
                    self.token[key] = data.data[key];
                }

                self.getEndpoints(function (err, data) {
                    if (data.success === true) {
                        self.token.metadata_url = data.data.metadataUrl;
                        self.token.content_url = data.data.contentUrl;
...
Account.prototype.load = function (callback) {
    ...
    this.cache.loadConfigByEmail(self.email, function (err, data) {
        ...
        config = data.length === 0 ? {} : data[0];
        for (var key in config) {
            self.token[key] = config[key];
        }
...
Account.prototype.renewAuthorization = function (callback) {
    ...
    Request.post('https://api.amazon.com/auth/o2/token', {
        ...
        if (response.statusCode === 200) {
            retval.success = true;
            retval.last_authorized = Date.now();
        }
