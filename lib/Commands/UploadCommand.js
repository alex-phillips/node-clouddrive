          ...
          opts = {
            ...
            onFileUpload: (localPath, callback) => {
              if (this.config.get('cli.progressBars')) {
                ...
                lastRun = Date.now();
                bar = new ProgressBar(`:percent[:bar] :speed eta :etas (:uploaded / :filesize)`, {
                  complete: '=',
                  incomplete: ' ',
                  width: 20,
                  total: localFilesize,
              ...
              if (bar) {
                ...
                if (timeDiff >= this.config.get('cli.progressInterval') || bytesUploaded >= localFilesize) {
                  ...
                  bar.tick(bytesDispatched, {
                    speed: `${Utils.convertFileSize(Math.round(bytesDispatched / (timeDiff / 1000)), 2)}/s`,
            ...
            onFileComplete: (response, body, retval, data, callback) => {
              ...
              if (response) {
                if (!body) {
                  Logger.error(`Failed to upload file '${data.localPath}'. Invalid body return with status code ${response.statusCode}: ${body}`);
