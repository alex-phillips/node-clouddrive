          ...
          open: function(path, flags, cb) {
            ...
            Node.loadByPath(path, (err, node) => {
              ...
              if (node.isFile()) {
                ...
                console.log(`downloading ${node.getId()}`);
                node.download(pth.join(Command.getCacheDirectory(), 'download', node.getId()), {}, (err, result) => {
                  if (err) {
                    console.log('error: ', err);
                  ...
                  fs.open(pth.join(Command.getCacheDirectory(), 'download', node.getId()), 'r', (err, fd) => {
                    cb(0, fd);
                  });
                });
              }
          ...
          read: function(path, fd, buf, len, pos, cb) {
            ...
            Node.loadByPath(path, (err, node) => {
              ...
              return node.downloadChunk({
                ...
                //return fs.write(fd, retval.data.buffer, pos, null, () => {
                //  return fs.read(fd, buf, pos, len, pos, (err, bytesRead, buffer) => {
                //    cb(bytesRead);
                //  });
                //});
                //fs.write(fd, retval.data.buffer, pos, len, null, () => {
                //  return fs.read(fd, buf, null, len, pos, (err, bytesRead, buffer) => {
                //    cb(retval.data.bytesWritten);
                //  });
                //});
              });
            });
