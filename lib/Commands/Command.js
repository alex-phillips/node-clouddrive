...
Command.prototype.readConfig = function () {
    ...
    for (var key in Command.defaultConfig) {
        this.config.set(key, Command.defaultConfig[key].default);
    }
    ...
    for (var i in Command.defaultConfig) {
        var val = Command.defaultConfig[i].default;
        if (savedConfig.get(i) !== undefined) {
            val = savedConfig.get(i);
        }

        this.setConfigValue(i, val);
    }
};
