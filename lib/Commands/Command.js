...
class Command {
  ...
  execute(args, options) {
    ...
    try {
      Logger.verbose(`Running ${this.constructor.name}`);
  ...
  initialize(callback) {
    Logger.verbose('Initializing command.');
    this.initializeDatabase(err => {
      if (err) {
        return callback(err);
      }

      Logger.verbose('Successfully conncted to databse.');
      ...
      this.account.load(err => {
        ...
        Logger.verbose('Successfully loaded account.');
        ...
        if (this.params.offline === false) {
          return this.account.authorize(null, {}, (err, data) => {
            ...
            Logger.verbose('Successfully authenticated with Amazon account.');
