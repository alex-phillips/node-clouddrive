...
class Command {
  ...
  initialize(callback) {
    var self = this;
    self.initializeDatabase((err) => {
      if (err) {
        return callback(err);
      }

      self.account.load((err) => {
        if (err) {
          return callback(err);
        }

        if (self.params.offline === false) {
          return self.account.authorize(null, (err, data) => {
            if (err) {
              return callback(err);
            }
    ...
    switch (this.config.get('database.driver')) {
      case 'sqlite':
        ...
        new SQLite({
          client: 'sqlite3',
          connection: {
            filename: `${Command.getCacheDirectory()}/${self.config.get('email')}.db`
          }
        }, (err, cache) => {
