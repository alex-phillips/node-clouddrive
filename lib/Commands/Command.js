...
class Command {
  ...
  static list(nodes, options) {
    ...
    for (let i = 0; i < nodes.length; i++) {
      ...
      var now = new Date(),
        modified = new Date(node.get('modifiedDate')),
        month = Utils.pad(moment(modified).format('MMM'), 4),
        day = Utils.pad(moment(modified).format('D'), 2, 'left'),
        date = `${month} ${day}  ${moment(modified).format('YYYY')}`;

      if (now.getYear() === modified.getYear()) {
        date = `${month} ${day} ${Utils.pad(moment(modified).format('H:mm'), 5, 'left')}`;
      }
