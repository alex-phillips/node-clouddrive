...
class Command {
  ...
  async initialize() {
    ...
    if (this.params.offline === false) {
      let data = await this.account.authorize(null, {});
    ...
    switch (this.config.get('database.driver')) {
      case 'sqlite':
        ...
        this.account = new Account({
          email: this.config.get('auth.email'),
          cache: cache,
          clientId: this.config.get('auth.id'),
          clientSecret: this.config.get('auth.secret'),
        });
        break;
      case 'mysql':
        let MySQL = require('../Cache/MySQL');
        cache = await new MySQL({
          client: 'mysql',
          connection: {
