...
class TreeCommand extends Command {
  ...
  static buildAsciiTree(node, prefix, options, callback) {
    node.getChildren({
      ...
      async.forEachSeries(nodes, (node, callback) => {
        ...
        if (node.inTrash()) {
          Logger.info(itemPrefix + chalk.red(node.getName()));
        } else if (node.isFolder()) {
          Logger.info(itemPrefix + chalk.blue(node.getName()));
        } else {
          Logger.info(itemPrefix + node.getName());
        }
  ...
  static buildMarkdownTree(node, prefix, options, callback) {
    node.getChildren({
      ...
      async.forEachSeries(nodes, (node, callback) => {
        Logger.info(`${prefix}- ${node.getName()}`);
        if (node.isFolder() || options.showAssets) {
