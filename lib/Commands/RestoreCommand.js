...
class RestoreCommand extends Command {
  ...
  restoreNode(node, callback) {
    ...
    node.getPath((err, path) => {
      Command.log(`Attempting to restore ${node.getKind()} node "${path}" (${node.getId()})...`, 3);
      if (!node.inTrash()) {
      ...
      node.restore((err, result) => {
        ...
        if (result.success) {
          Command.info(`Restored node "${path}" (${node.getId()})`);
        } else {
          Command.error(`Failed to restore node "${path}" (${node.getId()}): ${JSON.stringify(result)}`);
        }
