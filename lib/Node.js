    ...
    Request.get(account.contentUrl + "nodes/" + this.getId() + "/content", {
        ...
        .on('finish', function () {
            ...
            if (typeof options.onFileComplete === 'function') {
                options.onFileComplete(self, localPath, retval);
            }
...
Node.prototype.overwrite = function (localPath, callback) {
    ...
    var self = this;
    Request.put(account.contentUrl + "nodes/" + self.getId() + "/content", {
        headers: {
            Authorization: "Bearer " + account.token.access_token
        },
        formData: {
        ...
        Node.exists(remotePath + "/" + path.basename(localPath), localPath, function (err, data) {
            ...
            if (data.success === true) {
                ...
                if (data.data.pathMatch === true && data.data.md5Match === true) {
                    ...
                    if (typeof options.onFileComplete === 'function') {
                        options.onFileComplete(basename, remotePath, retval);
                    }
            ...
            var request = Request.post({
                ...
                if (response.statusCode === 201) {
                    ...
                    return account.authorize(null, function (data) {
                        if (data.success) {
                            return callback(null, retval);
                        }
            ...
            progressInterval = setInterval(function () {
                options.onFileProgress(request.req.connection._bytesDispatched);
            }, 1000);
        });
    });
};
