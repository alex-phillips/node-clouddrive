...
class Node extends ParameterBag {
  ...
  downloadFile(localPath, options, callback) {
    ...
    options.onFileDownload(this, () => {
      Logger.debug(`Requesting endpoint URL '${account.contentUrl}nodes/${this.getId()}/content'.`);
      let response = null,
  ...
  getChildren(options, callback) {
    ...
    Logger.debug(`Requesting endpoint URL '${account.metadataUrl}nodes/${this.getId()}/children'.`);
  ...
  getMetadata(generateLink, callback) {
    ...
    Logger.debug(`Requesting endpoint URL '${account.metadataUrl}nodes/${this.getId()}'.`);
  ...
  link(idParent, callback) {
    ...
    Logger.debug(`Requesting endpoint URL '${account.metadataUrl}nodes/${idParent}/children/${this.getId()}'.`);
  ...
  move(newParent, callback) {
    ...
    Logger.debug(`Requesting endpoint URL '${account.metadataUrl}nodes/${newParent.getId()}/children'.`);
  ...
  restore(callback) {
    ...
    Logger.debug(`Requesting endpoint URL '${account.metadataUrl}trash/${this.getId()}/restore'.`);
  ...
  trash(callback) {
    ...
    Logger.debug(`Requesting endpoint URL '${account.metadataUrl}trash/${this.getId()}'.`);
  ...
  static createFolder(name, parentId, callback) {
    ...
    Logger.debug(`Requesting endpoint URL '${account.metadataUrl}nodes'.`);
  ...
  static getTrash(callback) {
    Logger.debug(`Requesting endpoint URL '${account.metadataUrl}trash'.`);
  ...
  static restore(id, callback) {
    ...
    Logger.debug(`Requesting endpoint URL '${account.metadataUrl}trash/${id}/restore'.`);
    ...
    Node.createDirectoryPath(remotePath, (err, data) => {
      ...
      Node.exists(`${remotePath}/${path.basename(localPath)}`, localPath, options, (err, data) => {
        ...
        uploadOptions.onFileUpload(localPath, () => {
          ...
          Logger.debug(`Requesting endpoint URL '${account.contentUrl}nodes'.`);
