...
class Node extends ParameterBag {
  ...
  downloadFile(localPath, options, callback) {
    ...
    if (downloadOptions.decrypt) {
      Logger.verbose(`Creating decipher with password ${downloadOptions.password}`);
      let decipher = crypto.createDecipher(algorithm, downloadOptions.password);
      request.pipe(decipher).pipe(stream);
    } else {
      request.pipe(stream);
    }
  }
  ...
  static getUploadStream(path, options, callback) {
    ...
    Logger.debug(`Encrypting upload stream`);
    let cipher = crypto.createCipher(algorithm, options.password),
      tmpFile = `${path}.enc`;

    stream.pipe(cipher).pipe(fs.createWriteStream(tmpFile))
      .on('finish', () => {
