...
class Node extends ParameterBag {
  ...
  downloadFile(localPath, options, callback) {
        ...
        if (fs.existsSync(localPath)) {
          ...
          if (downloadOptions.checkMd5) {
            return Utils.getFileMd5(localPath, (err, md5) => {
              ...
              return downloadOptions.onFileComplete(null, null, retval, {
                node: this,
                localPath: localPath
              }, () => {
    ...
    downloadOptions.onFileDownload(this, () => {
      ...
      let response = null,
        ...
        .on('end', () => {
          ...
          return downloadOptions.onFileComplete(response, null, retval, {
            node: this,
            localPath: localPath
          }, () => {
            callback(null, retval);
      ...
      if (downloadOptions.onFileProgress) {
        request.on('data', data => {
          downloadOptions.onFileProgress(data);
        });
      }
  ...
  getChildren(options, callback) {
    ...
    Logger.verbse('Requesting nodes:folders:list endpoint');
    ...
    Node.createDirectoryPath(remotePath, (err, data) => {
      ...
      Node.exists(`${remotePath}/${path.basename(localPath)}`, localPath, options, (err, data) => {
        ...
        uploadOptions.onFileUpload(localPath, () => {
          ...
          Logger.verbse('Requesting nodes:files:upload endpoint');
